...
SceneFlowEntry = namedtuple("SceneFlowEntry",
                            ("frame_final", "frame_clean")) # , "optical_flow", "reverse_optical_flow", "motion_boundaries"))
...
class SceneFlowDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {
            "frame_final": Image.open(entry.frame_final).convert("RGB"),
            "frame_clean": Image.open(entry.frame_clean).convert("RGB"),
        ...
        for dirpath, dirnames, filenames in os.walk(os.path.join(root_dir, "frames_finalpass")):
            ...
            for i in range(1, len(filenames)):
                yield SceneFlowEntry(
                    ...
                    os.path.join(dirpath, filenames[i]).replace("frames_finalpass", "frames_cleanpass")
