...
def train(args):
    ...
    for e in range(args.epochs):
        ...
        for batch_id, (x, _) in enumerate(train_loader):
            ...
            if (args.feats):
                # x_feat = kornia.feature.dog_response_single(x, sigma1=1.0, sigma2=1.6)
                x_feat = kornia.feature.hessian_response(x, grads_mode='sobel', sigmas=None)
                # x_numpy = x_dog[0].permute(1,2,0).cpu().numpy()
                # plt.imshow(x_numpy)
                # plt.show()
                # y_feat = kornia.feature.dog_response_single(y, sigma1=1.0, sigma2=1.6)
                y_feat = kornia.feature.hessian_response(y, grads_mode='sobel', sigmas=None)
                feat_loss = args.depth_weight * mse_loss(y_feat, x_feat)
                ...
                if ((not args.depth_loss) and (not args.sem_loss)):
                    ...
                    mesg = "{}\tEpoch {}:\t[{}/{}]\tcontent: {:.6f}\tstyle: {:.6f}\tdepth: {:.6f}\ttotal: {:.6f}".format(
                        time.ctime(), e + 1, count, len(train_dataset),
                                    ...
                                    (agg_content_loss + agg_style_loss + agg_depth_loss) / (batch_id + 1)
                    )
                    # writer.add_scalar("content_loss", agg_content_loss / (batch_id + 1), log_count)
                    # writer.add_scalar("style_loss", agg_style_loss / (batch_id + 1), log_count)
...
def stylize_p(content_img, model, cuda=1, content_path='',content_scale=None):
    ...
    with torch.no_grad():
        style_model = TransformerNetLight() # TransformerNetDepth()
    ...
    output_image_name = 'images/output/' + str(
        model).replace('saved_models/', '').replace('to_onnx','').replace('.pth', '/') + str(
...
def main():
    ...
    train_arg_parser.add_argument("--depth-weight", type=float, default=1e3,
                                help="weight for depth-loss, default is 1e10")
    ...
    train_arg_parser.add_argument("--feats", type=int, default=1,
