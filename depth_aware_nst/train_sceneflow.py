    ...
    for e in range(args.epochs):
        ...
        with tqdm(train_loader, unit="batch") as tepoch:
            ...
            for sample in tepoch:
                ...
                x_final = sample["frame_final"].to(device)
                x_clean = sample["frame_clean"].to(device)

                y_final = transformer(x_final)
                y_clean = transformer(x_clean)

                img_grid = y_clean
                y_final = utils.normalize_batch(y_final)
                y_clean = utils.normalize_batch(y_clean)
                x_final = utils.normalize_batch(x_final)
                x_clean = utils.normalize_batch(x_clean)

                features_y_final = vgg(y_final)
                features_y_clean = vgg(y_clean)
                features_x_final = vgg(x_final)
                features_x_clean = vgg(x_clean)

                content_loss = args.content_weight * mse_loss(features_y_final.relu2_2, features_x_final.relu2_2)
                
                style_loss = 0.
                for ft_y, gm_s in zip(features_y_final, gram_style):
                ...
                # depth loss
                if (args.depth_loss): 
                    x_midas = midas(x_clean)
                    y_midas = midas(y_clean)
                ...
                if (args.sem_loss):
                    ## semantic
                    x_sem = deeplab(x_final)['out'][0]
                    y_sem = deeplab(y_final)['out'][0]
...
def main():
    ...
    train_arg_parser.add_argument("--pretrained-model", type=str, default='saved_models/starry_night.pth',
